...
async function fetchShopAndProduct(shopSlug: string, id: string) {
  ...
  const shopRes = await (supabase.from('shops') as any)
    .select('id, name, primary_color, plan, currency, country, target_countries, accept_cash_on_delivery, stripe_connect_enabled, logo_url, grid_image_ratio, city, address, delivery_zones, phone_whatsapp')
    .eq('slug', shopSlug)
    .single()
...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const hasOnlinePayment = onlineMethods.length > 0 || shop.stripe_connect_enabled
            ...
            {(hasOnlinePayment || acceptCash) && (
              <div className="mt-3 flex items-start gap-2 opacity-80">
                ...
                <div className="flex items-end gap-3 flex-wrap">
                  ...
                  {shop.stripe_connect_enabled && onlineMethods.length === 0 && (
                    <>

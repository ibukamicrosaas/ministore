...
export default async function ClientsPage({ searchParams }: Props) {
  ...
  const bronzeCount = clients.filter(c => (c.total_orders ?? 0) >= 1 && (c.total_orders ?? 0) < 3).length
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Résumé fidélité — section 9 de la spec : Or/Argent/Bronze ensemble,
          uniquement si au moins un client fidèle (jamais de carte à "0"). */}
      {!q && total > 0 && (goldCount > 0 || silverCount > 0 || bronzeCount > 0) && (
        <div className="flex gap-2">
          ...
          {bronzeCount > 0 && (
            <div className="flex items-center gap-1.5 rounded-xl border border-orange-200 bg-orange-50 px-3 py-2">
              <span className="text-sm">🥉</span>
              <div>
                <p className="text-xs font-bold text-orange-800">{bronzeCount} client{bronzeCount > 1 ? 's' : ''} Bronze</p>
                <p className="text-[10px] text-orange-600">1-2 commandes</p>
              </div>
            </div>
          )}
        </div>
            ...
            {clients.map((client) => {
              ...
              return (
                <div key={client.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  {/* Section 9 de la spec : un client à 0 commande n'a besoin
                      ni de badge (déjà absent, getLoyaltyTier renvoie null)
                      ni de ligne de statistique — juste son nom et son

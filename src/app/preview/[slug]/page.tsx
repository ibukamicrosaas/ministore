...
export default async function PreviewPage({ params }: Props) {
  ...
  const { data: rawShop } = await (supabase as any)
    .from('shops')
    .select('id, name, description, logo_url, primary_color, city, address, phone_whatsapp, available_days, delivery_options, plan, currency, cover_image_url, about_photo_url, business_category, badges, social_links, product_layout, is_active, trial_ends_at')
    .eq('slug', slug)
    .single() as {
      data: (Shop & {
        id: string

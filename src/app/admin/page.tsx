...
export default async function AdminOverviewPage() {
  ...
  const [
    ...
    supabase.from('payments').select('amount, created_at, shops!inner(country)').eq('status', 'completed'),
  ...
  const tekkishopCommission = payments.reduce((s, p) => {
    ...
    return s + Math.floor(p.amount * (getCommissionRate(shop?.country) / 100))
  }, 0)

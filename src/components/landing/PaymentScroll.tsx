...
const LOGOS = [
  ...
  { src: '/logo-payments/visa.svg', name: 'Visa' },
  { src: '/logo-payments/Mastercard-Logo.wine.svg', name: 'Mastercard' },
]
    ...
    <>
      ...
      <div className="md:hidden overflow-hidden">
        <div className="payment-track flex gap-3" style={{ width: 'max-content' }}>
          {doubled.map((l, i) => <LogoBadge key={i} src={l.src} name={l.name} />)}
        </div>
      </div>

      {/* Desktop — grille centrée */}
      <div className="hidden md:flex flex-wrap items-center justify-center gap-4">
        {LOGOS.map((l) => <LogoBadge key={l.name} src={l.src} name={l.name} />)}
      </div>
    </>
  )
}

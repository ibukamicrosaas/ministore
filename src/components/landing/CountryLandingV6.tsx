...
export async function CountryLandingV6({ config: cfg }: { config: CountryConfig }) {
  ...
  return (
    <div className="lv6">
        ...
        <div className="container hero-grid">
          <div className="hero-copy">
            <div className="social-proof">
              ...
              <div>
                <b>{count > 0 ? `+${count} boutiques créées` : `Nouveau ${cfg.hero.highlightedText}`}</b>
                <span>{cfg.hero.highlightedText}</span>
              </div>
            </div>

            <h1>
              {cfg.hero.headline.split(cfg.hero.highlightedText)[0]}
              <span className="grad">{cfg.hero.highlightedText}</span>
              {cfg.hero.headline.split(cfg.hero.highlightedText)[1]}
            </h1>

            <p className="lead">{cfg.hero.subtext}</p>
          ...
          <div className="hero-stage">
            ...
            <div className="float float-1">
              <i style={{ background: '#fff', overflow: 'hidden', padding: 4, boxSizing: 'border-box' }}>
                {cfg.paymentSection.logos[0].src ? (
                  <Image src={cfg.paymentSection.logos[0].src} alt={cfg.paymentSection.logos[0].name} width={21} height={21} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                ) : (
                  <span style={{ fontSize: 16 }}>{cfg.paymentSection.logos[0].icon}</span>
                )}
              </i>
              <div>
                <b>{cfg.hero.badge1.text}</b>
                <small>{cfg.hero.badge1.sub}</small>
              </div>
            </div>
            <div className="float float-2">
              <i style={{ background: '#176bff' }}>{IC.shoppingBag}</i>
              <div>
                <b>{cfg.hero.badge3Text}</b>
                <small>en 1 clic, sur WhatsApp</small>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── Bandeau paiements — spécifique au pays ──────────────────── */}
      <div className="strip">
        <div className="strip-label">{cfg.paymentSection.label}</div>
        <div className="pay-row">
          {cfg.paymentSection.logos.map((logo) => (
            <span className="pay" key={logo.name}>
              {logo.src ? (
                <Image src={logo.src} alt="" aria-hidden width={60} height={20} style={{ height: 20, width: 'auto' }} />
              ) : (
                <span aria-hidden style={{ fontSize: 18 }}>{logo.icon}</span>
              )}
              {logo.name}
            </span>
          ))}
      ...
      {/* ── Le problème — générique, identique à la landing ─────────── */}
      ...
      {/* ── Les 5 étapes — numéro local dans la démo ────────────────── */}
            ...
            <div className="step-media">
              <div className="app">
                ...
                <div className="app-in">
                  ...
                  <div className="qinput"><span data-type={cfg.steps.boutiqueName} data-delay="900" data-caret="1" /></div>
                  <div className="qchips"><span>{cfg.steps.boutiqueName}</span><span>Maison Kora</span><span>Elite Sneakers</span></div>
                  <div className="tip" data-appear="1900">L&rsquo;URL de ta boutique sera : <b>tekki.shop/{cfg.steps.slug}</b>. Choisis un nom court et mémorable que tes clients retiendront facilement.</div>
              ...
              <div className="app">
                ...
                <div className="app-in">
                  ...
                  <div className="fld" style={{ marginBottom: 11 }}>
                    ...
                    <b data-type={cfg.steps.phone} data-delay="800" />
                  </div>
                ...
                <div className="app-in">
                  <div className="sett-logo-row" data-appear="300">
                    ...
                    <div className="sett-logo-info">
                      <b>{cfg.steps.boutiqueName}</b>
                      <small>Logo de la boutique</small>
                      <span className="sett-change">Changer le logo</span>
                    </div>
                  </div>
                  <div className="fld"><small>Nom de la boutique *</small><b>{cfg.steps.boutiqueName}</b></div>
                  <div className="fld-row">
                    <div className="fld"><small>Ville *</small><b>{city}</b></div>
                    <div className="fld"><small>Pays</small><b>{cfg.name}</b></div>
                  </div>
              ...
              <div className="app">
                ...
                <div className="app-in">
                  <div className="db-link" data-appear="200">
                    ...
                    <div className="db-url">https://www.tekki.shop/{cfg.steps.slug} <span>↗</span></div>
      ...
      {/* ── Une vraie boutique — générique, identique à la landing ──── */}
      ...
      {/* ── Timeline 24h/24 — journée type du pays ──────────────────── */}
      <section className="timeline-sec">
        <div className="container">
          <div className="section-head center reveal">
            <span className="label label--dark">{cfg.journee.badge}</span>
            <h2>Ta boutique travaille pour toi, <span style={{ color: 'var(--amber)' }}>24h/24.</span></h2>
            <p className="lead">{cfg.journee.subtitle}</p>
          </div>
          <div className="tl-grid">
            <div className="tl reveal">
              {cfg.journee.items.map((row) => (
                <div key={row.heure} className="tl-row">
                  <div className="tl-time">{row.heure}</div>
      ...
      {/* ── Espace vendeur — étude de cas réelle, identique à la landing ── */}
      <section className="section">
        <div className="container split">
          <div className="split-media reveal">
            <div className="db-wrap">
      ...
      {/* ── Ce que tu peux vendre — générique, identique à la landing ── */}
        ...
        <div className="container">
          ...
          <div className="stats-bar reveal">
            {cfg.testimonialSection.stats.map((s, i) => (
              <div className="stat" key={i}><b>{s.value}</b><small>{s.label}</small></div>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* ── FAQ — partagée, déjà juste pour tous les pays couverts ──── */}
      <FAQv6 />
      ...
      <section className="cta-sec">
        <div className="container">
          <div className="cta-card reveal">
            <h2>{cfg.cta.headline}</h2>
            <p>{cfg.cta.subtitle}</p>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          mainEntity: [
            ...
            { '@type': 'Question', name: `Comment les clients paient-ils ${cfg.hero.highlightedText} ?`, acceptedAnswer: { '@type': 'Answer', text: cfg.paymentSection.label.replace(/\s*:\s*$/, '.') } },
          ],
        })}}
      />
    </div>

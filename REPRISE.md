...
**Si c'est bien le cas, la conséquence dépasse les boutiques de test.** Par défaut Vercel applique une variable à tous les environnements sauf restriction explicite — **cela vaudrait aussi pour `BICTORYS_API_KEY`/`STRIPE_SECRET_KEY`**. Une commande en ligne passée pendant la recette pourrait alors déclencher un vrai débit chez un vrai client de test, pas une transaction sandbox. **À vérifier avant toute étape de recette impliquant un paiement réel** — si ces clés sont des clés live (pas des clés de test/sandbox Bictorys ou Stripe), soit les éviter pendant la recette, soit confirmer qu'elles sont bien scindées par environnement avant de payer quoi que ce soit.
